...
public class AndroidTest extends InstrumentationTestCase {
	...
	// signatures too. Regression test for go-aft issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altoafter
	...
	workspace, err := ioutil.TempDir("", "aft-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/ethereum/afttest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.ethereum", "github.com/bananafund/AFToken/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "aft.aar"), "aft.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "ethereum", "afttest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
